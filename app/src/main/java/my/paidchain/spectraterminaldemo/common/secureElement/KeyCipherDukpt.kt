...
abstract class KeyCipherDukpt(private val app: Application, private val controller: KeyDllController) {
    internal fun encrypt(keySlot: KeySlot, params: KeyParam): ByteArray {
        ...
        val result = when (getKeyValueOrNull(params, KeyParamKey.CIPHER_MODE.name) as CipherMode?) {
            null -> controller.cipher(app, keySlot.index, KKeyDllController.CipherModeConstant.K_Encrypt, data, null, ksn)

            CipherMode.CBC -> {
                val iv: ByteArray = getKeyValue(params, KeyParamKey.IV.name)
                controller.cipher(app, keySlot.index, (KKeyDllController.CipherModeConstant.K_Encrypt.toInt() or CipherMode.CBC.value).toShort(), data, iv, ksn)
            }
    ...
    internal fun decrypt(keySlot: KeySlot, params: KeyParam): ByteArray {
        ...
        val result = when (getKeyValueOrNull(params, KeyParamKey.CIPHER_MODE.name) as CipherMode?) {
            null -> controller.cipher(app, keySlot.index, KKeyDllController.CipherModeConstant.K_Decrypt, data, null, ksn)

            CipherMode.CBC -> {
                val iv: ByteArray = getKeyValue(params, KeyParamKey.IV.name)
                controller.cipher(app, keySlot.index, (KKeyDllController.CipherModeConstant.K_Decrypt.toInt() or CipherMode.CBC.value).toShort(), data, iv, ksn)
            }
