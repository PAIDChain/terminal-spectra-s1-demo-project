...
abstract class KeyCipherAes(private val app: Application, private val controller: KeyDllController) {
    internal fun encrypt(keySlot: KeySlot, params: KeyParam): ByteArray {
        ...
        val result = when (getKeyValueOrNull(params, KeyParamKey.CIPHER_MODE.name) as CipherMode?) {
            ...
            CipherMode.CBC -> {
                ...
                val cipherMode = (KKeyDllController.CipherModeConstant.K_Encrypt.toInt() or CipherMode.CBC.value).toShort()

                controller.cipher(app, keySlot.index, cipherMode, data, iv)
            }
    ...
    internal fun decrypt(keySlot: KeySlot, params: KeyParam): ByteArray {
        ...
        val result = when (getKeyValueOrNull(params, KeyParamKey.CIPHER_MODE.name) as CipherMode?) {
            ...
            CipherMode.CBC -> {
                val iv: ByteArray = getKeyValue(params, KeyParamKey.IV.name)
                val cipherMode = (KKeyDllController.CipherModeConstant.K_Decrypt.toInt() or CipherMode.CBC.value).toShort()
